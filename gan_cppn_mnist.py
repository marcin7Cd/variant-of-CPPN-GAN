...
class Generator(nn.Module):
    def __init__(self, x_dim, y_dim, batch_size=1, z_dim = 32, c_dim = 1, scale = 8.0, net_size = 128, devid = -1,):
        ...
        self.lin_seq = nn.Sequential(nn.ReLU(True), self.linear5, nn.ReLU(True), self.linear6, nn.ReLU(True),
                                 self.linear7, nn.ReLU(True), self.linear8, self.hardtanh)
...
if __name__ == "__main__":
    ...
    for iteration in range(ITERS):
        ...
        lib.plot.plot('tmp\\mnist\\time', time.time() - start_time)
        lib.plot.plot('tmp\\mnist\\train disc cost', D_cost.cpu().data.numpy())
        lib.plot.plot('tmp\\mnist\\train gen cost', G_cost.cpu().data.numpy())
