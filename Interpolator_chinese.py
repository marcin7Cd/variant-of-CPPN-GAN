...
def interpolate(state_dict, generator, preview = True, interpolate = False, large_sample=False,
    ...
    if preview:
        ...
        print(noisev.shape)
        ones = torch.ones(BATCH_SIZE, x_large * y_large, c_d)
        ...
        imageio.mimsave('generated_img/movie_c{}im{}_{}x{}.gif'.format(counter,x_large-x_large//64+1,
                                                                       disp_size,disp_size),
                        images)
        print('saved')
...
if __name__ == "__main__":
    ...
    for counter in range(19,20):
        interpolate("tmp/chinese/G-cppn-wgan_"+str(counter*10000)+".pth", G,
                    preview=False, large_sample=False,
                    disp_size=6,
                    interpolate=True, large_dim=64,
                    samples=[4,5,7,8,11,19,21,28,31],counter=counter)  
